...
export const Image = (path, option) =>{
    ...
    const urlRegx = /https?:\/\/[-_.!~*'()a-zA-Z0-9;/?:@&=+$,%#\u3000-\u30FE\u4E00-\u9FA0\uFF01-\uFFE3]+/g
    if (path.includes('%PUBLIC_IMAGE%')) {
        ...
        return ( /*html*/ `<img src="${correctedPath}" width="${width}" height="${height}">`)
    } else if (path.match(urlRegx)) {
        return ( /*html*/ `<img src=${path} width="${width}" height="${height}">`)
    } else {
        return ( /*html*/ `<img src="${UPPER_PATH + path}">`)
    }
}